...
public:
    ...
    template <typename Func>
    constexpr auto map_adjacent(Func func) &&;
...
struct map_adjacent_adaptor : flow_base<map_adjacent_adaptor<Base, Func>> {
private:
    Base flow1_;
    using Base2 = decltype(flow1_.subflow().drop(1));
    Base2 flow2_;
    FLOW_NO_UNIQUE_ADDRESS Func func_;

    using item_type = std::invoke_result_t<Func&, item_t<Base>, item_t<Base2>>;

public:
    constexpr map_adjacent_adaptor(Base&& base, Func&& func)
        : flow1_(std::move(base)),
          flow2_(flow1_.subflow().drop(1)),
          func_(std::move(func))
    {}
...
template <typename D>
template <typename Func>
constexpr auto flow_base<D>::map_adjacent(Func func) &&
{
    static_assert(is_multipass_flow<D>,
                  "map_adjacent requires a multipass flow");
    return detail::map_adjacent_adaptor<D, Func>(consume(), std::move(func));
}
